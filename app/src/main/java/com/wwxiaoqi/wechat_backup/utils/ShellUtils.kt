    ...
    fun execCmd(commands: Array<String>?, isRoot: Boolean, isNeedResultMsg: Boolean = true): CommandResult {
      ...
      try {
        ...
        if (isNeedResultMsg) {
          ...
          while (successResult.ready()) {
            if (successResult.readLine().also { s = it } != null) {
              successMsg.append(s)
            }
          }

          while (errorResult.ready()) {
            if (errorResult.readLine().also { s = it } != null) {
              errorMsg.append(s)
            }
          }
        }
